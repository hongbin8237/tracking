...
FILE * trackFile = fopen("result_JH.txt", "w");
...
//		cvAdaptiveThreshold(imgC, imgC, 255, CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY_INV, 3, 0 );
		...
		for ( int i = 0; i < nf; i++){
			...
			distance[i] = sqrt((featuresA[i].x-featuresB[i].x)*(featuresA[i].x-featuresB[i].x) + (featuresA[i].y-featuresB[i].y)*(featuresA[i].y-featuresB[i].y));
			sumDistance = sumDistance+distance[i];
...
//cout << "i = " << i << ", duration = " << duration[i] << endl;
		}
		avgDistance = sumDistance/nf;
//		cout << "sumDistance = " << sumDistance << ", avgDistance = " << avgDistance << endl;
		for ( int i = 0; i < nf; i++){
//			cout << "distance[" << i << "] =  " << distance[i] << endl;
//			if (distance[i] > avgDistance*1.5){
			if (distance[i] > avgDistance+50){
				featuresA[i].x = 0;
				featuresA[i].y = 0;
				featuresB[i].x = 0;
				featuresB[i].y = 0;
...
//		cout << "cnt = " << cnt << endl;
		...
		for ( int i = 0; i < nf; i++){
			if (featuresB[i].x == 0){
				...
				//cout << "i = " << i << ", B = " << featuresB[i].x << ", C = " << featuresC[j].x << endl;
			}
			if (featuresB[i].x == 0){
				jj++;
				cout << "jj = " << jj << endl;
...
//			cvPutText(imgColor, text, cvPoint(q.x+2,q.y+2), &font, cvScalar(255,10,10));
